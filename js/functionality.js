        ...
        .then(
            (data) => {
                ...
                for (let i = 0; i < obj.length; i++) {
                    ...
                    obj[i].title = obj[i].title.replaceAll("'", "").replaceAll('"', '').replaceAll("'", "");
                    //  obj[i].title = obj[i].description.replaceAll("'", "&#39;").replace('"', '&#34;');
                    objHTML = objHTML + `<li  class='list-group-item ' data-num='${i}' ><label class=' pointer'  onClick="toggle(\'${obj[i].title.replaceAll("'", "").trim()}\')">${obj[i].title}</label><div class='hide' data-toggle='${obj[i].title.replaceAll("'", "").trim()}'><p>${obj[i].description}</p><small><i>${obj[i].pubDate} - <a href='${obj[i].link}' target='_blank'>Article Link</a></i></small></div></li>`
